...
export class NoticeService implements INoticeService {
  ...
  async add(params: INoticeService.AddParams): Promise<Notice> {
    ...
    noticeDTO.append(
      ...
      params.registrationStartDate.toISOString()
    );
    noticeDTO.append(
      "registrationEndDate",
      params.registrationEndDate.toISOString()
    );
    noticeDTO.append(
      "evaluationStartDate",
      params.evaluationStartDate.toISOString()
    );
    noticeDTO.append(
      "evaluationEndDate",
      params.evaluationEndDate.toISOString()
    );
    ...
    noticeDTO.append(
      "sendingDocsStartDate",
      params.sendingDocsStartDate.toISOString()
    );
    noticeDTO.append(
      "sendingDocsEndDate",
      params.sendingDocsEndDate.toISOString()
    );
    noticeDTO.append("suspensionYears", params.suspensionYears.toString());
    noticeDTO.append(
      "partialReportDeadline",
      params.partialReportDeadline.toISOString()
    );
    noticeDTO.append(
      "finalReportDeadline",
      params.finalReportDeadline.toISOString()
    );
      ...
      params.activities.forEach((activityType, atIndex) => {
        ...
        if (activityType.activities) {
          activityType.activities.forEach((activity, index) => {
            noticeDTO.append(
              `activities[${atIndex}].activities[${index}].name`,
              activity.name
            );
            noticeDTO.append(
              `activities[${atIndex}].activities[${index}].points`,
              activity.points.toString()
            );
            noticeDTO.append(
              `activities[${atIndex}].activities[${index}].limits`,
              activity.limits.toString()
            );
          });
        }
      });
    ...
    try {
      ...
      throw new Error(
        `Erro ao realizar o cadastro de edital: ${error.message}`
      );
    }
  }
  ...
  async update(params: INoticeService.UpdateParams): Promise<Notice> {
    ...
    noticeDTO.append(
      "registrationStartDate",
      typeof params.registrationStartDate === "string"
        ? params.registrationStartDate
        : params.registrationStartDate.toISOString()
    );
    noticeDTO.append(
      "registrationEndDate",
      typeof params.registrationEndDate === "string"
        ? params.registrationEndDate
        : params.registrationEndDate.toISOString()
    );
    noticeDTO.append(
      "evaluationStartDate",
      typeof params.evaluationStartDate === "string"
        ? params.evaluationStartDate
        : params.evaluationStartDate.toISOString()
    );
    noticeDTO.append(
      "evaluationEndDate",
      typeof params.evaluationEndDate === "string"
        ? params.evaluationEndDate
        : params.evaluationEndDate.toISOString()
    );
    noticeDTO.append(
      "appealStartDate",
      typeof params.appealStartDate === "string"
        ? params.appealStartDate
        : params.appealStartDate.toISOString()
    );
    noticeDTO.append(
      "appealEndDate",
      typeof params.appealEndDate === "string"
        ? params.appealEndDate
        : params.appealEndDate.toISOString()
    );
    noticeDTO.append(
      "sendingDocsStartDate",
      typeof params.sendingDocsStartDate === "string"
        ? params.sendingDocsStartDate
        : params.sendingDocsStartDate.toISOString()
    );
    noticeDTO.append(
      "sendingDocsEndDate",
      typeof params.sendingDocsEndDate === "string"
        ? params.sendingDocsEndDate
        : params.sendingDocsEndDate.toISOString()
    );
    noticeDTO.append(
      "partialReportDeadline",
      typeof params.partialReportDeadline === "string"
        ? params.partialReportDeadline
        : params.partialReportDeadline.toISOString()
    );
    noticeDTO.append(
      "finalReportDeadline",
      typeof params.finalReportDeadline === "string"
        ? params.finalReportDeadline
        : params.finalReportDeadline.toISOString()
    );
    noticeDTO.append("suspensionYears", params.suspensionYears.toString());
    if (params.attachedFile) noticeDTO.append("file", params.attachedFile);
    ...
    try {
      ...
      throw new Error(
        `Erro ao realizar a atualização de edital: ${error.message}`
      );
    }
  }
